...
namespace OPM.SFS.Web.Pages.Admin
{
    public class CertLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGet()
        {
            ...
            if (pivAuth.IsSuccess) return RedirectToPage("/Admin/Home");
            if (pivAuth.RegistrationRequired) return RedirectToPage("/Admin/RegisterPIV", new { c = pivAuth.RegistrationStagingID });
            if (pivAuth.IsInactive) return Redirect($"/Admin/Login?ia=true&ra={Uri.EscapeDataString(pivAuth.EncryptedUserID)}");
            ModelState.AddModelError("UserId", pivAuth.ErrorMessage);
            return RedirectToPage("/Admin/CertificateError");
        }
        ...
        public class LoginPIVHandler : IRequestHandler<LoginCommandPIV, PIVLoginResult>
        {
            ...
            private readonly IAuditEventLogHelper _auditor;
            private readonly ICryptoHelper _crypto;

            public LoginPIVHandler(ScholarshipForServiceContext efDB, ICacheHelper cache, IHttpContextAccessor httpContextAccessor, IAuditEventLogHelper auditor, ICryptoHelper crypto)
            {
                _efDB = efDB;
                _cache = cache;
            ...
            public async Task<PIVLoginResult> Handle(LoginCommandPIV request, CancellationToken cancellationToken)
            {
                ...
                await _auditor.LogAuditEvent($"PIV Cert:{request.CertificateString}");
                ...
                if (myMatch.Success)
                {
                    ...
                    var certUser = await _efDB.AdminUsers.Where(m => m.Thumbprint == theCert.GetCertHash()).Include(m => m.AdminUserRole).FirstOrDefaultAsync();
                    if (certUser != null && certUser.IsDisabled)
                    {
                        var GlobalConfigSettings = await _cache.GetGlobalConfigurationsAsync();
                        return new PIVLoginResult() { IsInactive = true, IsSuccess = false, ErrorMessage = "", EncryptedUserID = EncryptString(certUser.AdminUserId.ToString(), GlobalConfigSettings) };
                    }
                    ...
                    {                      
                        ...
                        {
                            new Claim(ClaimTypes.NameIdentifier, certUser.AdminUserId.ToString()),
                            new Claim("SFS_UserID", certUser.AdminUserId.ToString()),
                            new Claim(ClaimTypes.Name, certUser.FirstName),
                            new Claim("AdminRole", certUser.AdminUserRole.Role),
                            new Claim(ClaimTypes.Role, "AD"),
                            new Claim(ClaimTypes.AuthenticationMethod, "Smartcard")
                        };
                        await _auditor.LogAuditEvent($"PIV: User {certUser.AdminUserId} Role AD logged in successfully.");
